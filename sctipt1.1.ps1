...
#Ver 1.1
#Revised 12/14/2021
#Added additional comments and allowed the user to keep running the program without having to relaunch it everytime it sends emails.

#Do-while loop that allows the program to keep running/restart as necessary
Do{

#Flag that allows the program to keep running.
$appFlag = 0
...
#optionFlag that is a condition in running the below while loop to allow the user to enter data, or re-enter it should they make a mistake when confirming.
$optionFlag = 0

##While loop that allows the user to confirm their selection and allows the user to make sure they are choosing either yes or not before terminating the application or sending the emails.
...
{
#Informs the user that they mistypese their unput while allowing them to press any key, keeping the optionFlag at 0 so the user can correctly type their response without having to restart the program.
...
##Else statement for the If statement that checks that both emails answered are correct, if they do not equal, the below code block executes.
else
{
#Informs the user that the emails do not match and allows the user to presss any key and clears the screen to restart the program.
